...
const POLICY_HTML_CLASS =
  'storefront-policy-html text-[14px] leading-7 text-[#121212] [&_a]:text-[#1773b0] [&_a]:underline [&_blockquote]:my-3 [&_blockquote]:border-l-4 [&_blockquote]:border-[#dedede] [&_blockquote]:pl-3 [&_h1]:mb-3 [&_h1]:text-2xl [&_h1]:font-bold [&_h2]:mb-2 [&_h2]:text-xl [&_h2]:font-semibold [&_h3]:mb-2 [&_h3]:text-lg [&_h3]:font-semibold [&_li]:my-1 [&_ol]:my-3 [&_ol]:list-decimal [&_ol]:pl-6 [&_p]:my-3 [&_ul]:my-3 [&_ul]:list-disc [&_ul]:pl-6';

export function StorefrontPolicyModal({ open, title, loading = false, error = null, content, onClose }: Props) {
  ...
  return createPortal(
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="storefront-policy-modal-title"
      >
        <div className="flex shrink-0 items-center justify-between border-b border-[#e1e3e5] px-5 py-4">
          <h2 id="storefront-policy-modal-title" className="text-[16px] font-semibold text-[#121212]">
            {title}
          </h2>
          <button
            type="button"
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
          {loading ? (
            ...
            <div className={POLICY_HTML_CLASS} dangerouslySetInnerHTML={{ __html: content }} />
          ) : (
