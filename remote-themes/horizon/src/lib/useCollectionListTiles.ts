...
import { collectionPath, STOREFRONT_PATHS } from './storefrontPaths';
...
function parseCollectionLinksPicker(raw: string): string[] {
  if (!raw.trim()) return [];
  return raw
    .split(/[,\n]/)
...
function mapStorefrontCollectionsToTiles(
  ...
  const existingByHandle = new Map(
    configTiles.map((tile) => {
      const handle = tile.href.replace(/^\/collections\//, '').replace(/^\/collection\//, '');
  ...
  return collections.map((col, index) => {
    ...
    const href = handle ? collectionPath(handle) : STOREFRONT_PATHS.allProducts;
...
export function useCollectionListTiles(
  ...
  }, [collections, config, configTiles, placement, sectionId, settingsBase, templateId]);
}
