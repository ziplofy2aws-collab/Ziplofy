...
import { blogTagsRouter } from './routes/blog-tags.route';
import { blogCommentRouter } from './routes/blog-comment.route';
import { contactFormSubmissionRouter } from './routes/contact-form-submission.route';
...
import { giftCardProductRouter } from './routes/gift-card-product.route';
...
import informaticThemeRouter from './routes/informatic-theme.route';
...
import onlineStorePreferencesRouter from './routes/online-store-preferences.route';
...
import { paymentProviderRouter } from './routes/payment-provider.route';
...
import { storeFrontPageRouter } from './routes/storefront/storefront-page.route';
...
app.use("/api/informatic-themes", informaticThemeRouter);
...
app.use("/api/contact-form-submissions", contactFormSubmissionRouter);
...
app.use("/api/storefront/pages", storeFrontPageRouter);
...
app.use("/api/gift-card-products", giftCardProductRouter);
...
app.use("/api/online-store-preferences", onlineStorePreferencesRouter);
...
app.get('/api/health', (req: Request, res: Response) => {
  res.status(200).json({ 
    status: 'ok  - team codiic hello world ci cd updated 1234',
    timestamp: new Date().toISOString(),
    uptime: process.uptime()
  });
});
