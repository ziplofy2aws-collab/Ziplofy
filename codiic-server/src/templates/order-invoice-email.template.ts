...
export const ORDER_INVOICE_EMAIL_SUBJECT = 'Invoice #{{order_number}}';
...
export function buildSampleInvoiceOrderItemsHtml(): string {
  return [
...
export function buildInvoiceOrderTotalsHtml(options?: {
  subtotal?: string;
  discount?: string;
  discountCode?: string;
  shipping?: string;
...
export function buildOrderInvoiceEmailHtml(variables: Record<string, string>): string {
...
export function getOrderInvoiceEmailBodyTemplate(): string {
...
export const ORDER_INVOICE_EMAIL_BODY = getOrderInvoiceEmailBodyTemplate();
