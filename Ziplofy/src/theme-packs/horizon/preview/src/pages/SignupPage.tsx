...
const SEC = 'templates.signup.sections.signup_main';
...
export function SignupPage() {
  ...
  const firstPh = cfgString(config, `${SEC}.blocks.form_fields.blocks.first_name_field.settings.placeholder`);
  const lastPh = cfgString(config, `${SEC}.blocks.form_fields.blocks.last_name_field.settings.placeholder`);
  ...
  const promptText = cfgString(config, `${SEC}.blocks.footer_link.blocks.login_prompt.settings.text`);
  const linkLabel = cfgString(config, `${SEC}.blocks.footer_link.blocks.login_link.settings.label`);
  const linkHref = cfgString(config, `${SEC}.blocks.footer_link.blocks.login_link.settings.href`);
  ...
  return (
    <PageShell>
      <EditorSection sectionId="signup_main" label="Sign up form" style={{ padding: `48px ${layout.padX}px 80px`, fontFamily: fontBody, color: text }}>
        <div style={{ maxWidth: 440, margin: '0 auto', border: `1px solid ${layout.line}`, borderRadius: 12, padding: 40 }}>
          ...
          <form onSubmit={(e) => void handleSubmit(e)} style={{ display: 'grid', gap: 16 }}>
            <EditorBlock nodeId="template:signup:signup_main:block:form_fields" label="Form fields">
              <input value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder={firstPh} style={inputStyle} />
              <input value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder={lastPh} style={inputStyle} />
            ...
            <EditorBlock nodeId="template:signup:signup_main:block:primary_button" label="Submit button">
              <button type="submit" disabled={loading} style={{ marginTop: 8, background: primary, color: '#fff', border: 'none', padding: '14px 20px', borderRadius: 8, cursor: loading ? 'wait' : 'pointer', fontWeight: 600, width: '100%' }}>
                {loading ? btnLoading : btnLabel}
              </button>
            </EditorBlock>
          </form>
          <EditorBlock nodeId="template:signup:signup_main:block:footer_link" label="Sign in link">
            <p style={{ marginTop: 20, fontSize: 14 }}>
              <EditorField fieldPath={`${SEC}.blocks.footer_link.blocks.login_prompt.settings.text`} label="Prompt text" as="span">
                {promptText}{' '}
              </EditorField>
              <Link to={linkHref} style={{ color: primary, fontWeight: 600 }}>
                <EditorField fieldPath={`${SEC}.blocks.footer_link.blocks.login_link.settings.label`} label="Link label" as="span">
                  {linkLabel}
                </EditorField>
              </Link>
            </p>
