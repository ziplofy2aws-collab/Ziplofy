...
export function readNewsletterBlockStyle(
  ...
  return {
    ...
    submit: {
      style: cfgString(config, `${newsletterBase}.submitStyle`, 'link') === 'button' ? 'button' : 'link',
      display: cfgString(config, `${newsletterBase}.submitDisplay`) === 'text' ? 'text' : 'arrow',
      integrated: cfgBool(config, `${newsletterBase}.submitIntegratedButton`, true),
    },
    padding: {
