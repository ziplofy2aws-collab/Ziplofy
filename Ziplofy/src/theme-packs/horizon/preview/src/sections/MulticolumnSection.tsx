...
export function MulticolumnSection({
  ...
  const mobileCss = mobileStackClass ? multicolumnMobileStackCss(sectionId) : '';
  const blocksBase = settingsBase.replace(/\.settings$/, '');

  return (
    <EditorSection sectionId={sectionId} label="Multicolumn" editorNodeId={editorNodeId} style={shell}>
