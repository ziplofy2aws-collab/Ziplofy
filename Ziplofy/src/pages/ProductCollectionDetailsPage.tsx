...
import DuplicateCollectionModal from '../components/collections/DuplicateCollectionModal';
...
const ProductCollectionDetailsPage: React.FC = () => {
  ...
  const { collections, deleteCollection, duplicateCollection, updateCollection, loading: collectionsLoading } = useCollections();
  ...
  const handleThemeTemplateChange = useCallback(
    async (themeTemplate: string) => {
      if (!collection?._id || themeTemplate === (collection.themeTemplate || 'default')) return;
      try {
        await updateCollection(collection._id, { themeTemplate });
      } catch (error) {
        console.error('Failed to update collection theme template:', error);
      }
    },
    [collection?._id, collection?.themeTemplate, updateCollection]
  );
  ...
  const handleDuplicateCollection = useCallback(async () => {
    ...
    try {
      const duplicated = await duplicateCollection(collection._id);
      toast.success('Collection duplicated');
      setDuplicateOpen(false);
      navigate(`/products/collections/${duplicated._id}`);
    } catch (error: any) {
      toast.error(error?.response?.data?.message || error?.message || 'Failed to duplicate collection');
    } finally {
      setIsDuplicating(false);
    }
  }, [collection?._id, duplicateCollection, navigate]);
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12 lg:gap-8">
          ...
          <aside className="space-y-6 lg:col-span-4">
            ...
            <CollectionThemeTemplateSection
              ...
              value={collection.themeTemplate || 'default'}
              onChange={handleThemeTemplateChange}
              className="rounded-2xl border border-gray-200/80 bg-white p-5 shadow-sm sm:p-6"
              selectClassName="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
            />
            ...
            <section className="rounded-2xl border border-gray-200/80 bg-white p-5 shadow-sm sm:p-6">
              <button
                ...
                disabled={isDuplicating || collectionsLoading}
                className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <DocumentDuplicateIcon className="h-4 w-4" aria-hidden />
                {isDuplicating ? 'Duplicating…' : 'Duplicate collection'}
              </button>
            </section>
