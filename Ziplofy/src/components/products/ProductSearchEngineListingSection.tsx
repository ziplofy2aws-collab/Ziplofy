...
interface ProductSearchEngineListingSectionProps {
  ...
  /** URL segment before the handle, e.g. `products/` or `collections/` */
  urlPathPrefix?: string;
  /** Used in helper copy, e.g. `product` or `collection` */
...
function slugFromTitle(title: string, fallback = "product"): string {
  const s = title
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
...
> = ({
  ...
  const handleSlug = useMemo(() => {
    ...
    if (h) return h;
    return slugFromTitle(productTitle, resourceLabel);
  }, [urlHandle, productTitle, resourceLabel]);

  const baseOrigin =
    typeof window !== "undefined" ? window.location.origin : "";
  const normalizedPathPrefix = urlPathPrefix.endsWith("/")
    ? urlPathPrefix
    : `${urlPathPrefix}/`;
  const fullUrlPreview = `${baseOrigin}/${normalizedPathPrefix}${handleSlug}`;
  ...
  return (
    <div className="rounded-xl border border-gray-200/80 bg-white p-6 shadow-sm">
      ...
      {!editing ? (
        <div className="mt-4">
          {listingEmpty ? (
            <p className="text-sm text-gray-600">
              Add a title and description to see how this {resourceLabel} might appear
        ...
        <div className="mt-4">
          <p className="text-sm text-gray-600">
            Add a title and description to see how this {resourceLabel} might appear in
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={slugFromTitle(productTitle, resourceLabel)}
              ...
              <p className="mt-2 break-all text-sm text-gray-500">
                {fullUrlPreview || `${baseOrigin}/${normalizedPathPrefix}`}
              </p>
            </div>
          </div>
        </div>
