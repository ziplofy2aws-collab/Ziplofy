...
type ProductAddedBannerProps = {
  productTitle: string;
  onDismiss: () => void;
  onAddAnother: () => void;
};

const ProductAddedBanner: React.FC<ProductAddedBannerProps> = ({
  productTitle,
  onDismiss,
  onAddAnother,
}) => {
    ...
    >
      <div className="flex items-center justify-between gap-3 bg-emerald-700 px-4 py-2.5 text-white">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className="truncate text-[13px] font-semibold">Added {productTitle}</span>
        </div>
        <button
          type="button"
          onClick={onDismiss}
...
export default ProductAddedBanner;
