...
export const COLLECTION_LIST_HEADER_PANEL_GROUP_ORDER = [
  'Layout',
  'Size',
  'Appearance',
  'Borders',
...
const PANEL_GROUPS = new Set<string>(COLLECTION_LIST_HEADER_PANEL_GROUP_ORDER);
...
export function collectionListHeaderDefaultSettings(): Record<string, string | number | boolean> {
  return {
    direction: 'vertical',
...
export function pickCollectionListHeaderField(
...
function resolvePercentSliderField(
  ...
  const existing = pickCollectionListHeaderField(fields, key);
  if (existing) {
    return {
      ...existing,
      label,
...
export function resolveCollectionListHeaderCustomWidthField(
  fields: EditorFieldDef[],
  anchor: EditorFieldDef | undefined,
...
export function resolveCollectionListHeaderCustomHeightField(
  fields: EditorFieldDef[],
  anchor: EditorFieldDef | undefined
): EditorFieldDef | null {
...
export function isCollectionListHeaderPanelField(field: EditorFieldDef): boolean {
...
export function isCollectionListSectionHeaderPanelFields(fields: EditorFieldDef[]): boolean {
  ...
  return fields.every(isCollectionListHeaderPanelField);
}

export function groupCollectionListHeaderPanelFields(
  fields: EditorFieldDef[]
): Map<string, EditorFieldDef[]> {
  const map = new Map<string, EditorFieldDef[]>();
  for (const field of fields.filter(isCollectionListHeaderPanelField)) {
...
export const COLLECTION_LIST_HEADER_DEFAULTS: Record<string, string | boolean> = Object.fromEntries(
  Object.entries(collectionListHeaderDefaultSettings()).map(([k, v]) => [
    k,
