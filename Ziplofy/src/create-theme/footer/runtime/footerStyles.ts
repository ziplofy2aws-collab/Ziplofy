...
export function footerColorScheme(
  ...
  const key = cfgString(config, `${settingsBase}.colorScheme`, 'scheme-2');
...
export function footerPadding(config: Record<string, unknown> | null, settingsBase: string) {
  return {
    paddingTop: cfgNumber(config, `${settingsBase}.paddingTop`, 36),
    paddingBottom: cfgNumber(config, `${settingsBase}.paddingBottom`, 36),
  };
}
