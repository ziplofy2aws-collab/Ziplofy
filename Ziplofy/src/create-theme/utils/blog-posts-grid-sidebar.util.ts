...
import {
  ...
  isBlogPostsGridSectionInstanceId,
...
export const BLOG_POSTS_GRID_SECTION_CHILD_ORDER = ['title', 'blog_card'] as const;
export const BLOG_POSTS_GRID_CARD_CHILD_ORDER = ['image', 'title', 'details', 'excerpt'] as const;
export const BLOG_POSTS_GRID_DEFAULT_POST_IDS = ['post_1', 'post_2', 'post_3'] as const;
...
export function blogPostsGridTemplatePostId(
  config: Record<string, unknown> | null,
  blocksPath: string[],
  ...
  return ids[0] ?? BLOG_POSTS_GRID_DEFAULT_POST_IDS[0];
}

function blogPostsGridSectionBase(prefix: string): string {
...
/** Shopify Blog posts: Grid — Title; Blog card → Image / Title / Details / Excerpt. */
export function mapBlogPostsGridBlockNodes(
  prefix: string,
  blocksBase: string,
  values: Record<string, string | boolean>,
  ...
  const sectionBase = blogPostsGridSectionBase(prefix);
  const titlePrefix = `${prefix}:block:title`;
  const cardPrefix = `${prefix}:block:blog_card`;
  const blocksPath = blocksBase.split('.');
  const postId = blogPostsGridTemplatePostId(config, blocksPath, blockOrderPath);
...
export function blogPostsGridStructureOrder(
  prefix: string,
  sectionChildrenListKey: string,
...
export function blogPostsGridLayoutStructureOrder(
  prefix: string,
  sectionChildrenListKey: string,
  itemOrder: Record<string, string[]>
): Record<string, string[]> {
  return blogPostsGridStructureOrder(prefix, sectionChildrenListKey, itemOrder);
}
...
function blogPostsGridFieldSidebarNodeId(settingsBase: string, blockSuffix: string): string | null {
...
function blogPostsGridSettingsBaseFromFieldPath(path: string): string | null {
...
function blogPostsGridSectionSidebarNodeId(settingsBase: string): string | null {
  const tpl = settingsBase.match(/^templates\.([^.]+)\.sections\.([^.]+)\.settings$/);
...
export function isBlogPostsGridContentFieldPath(path: string): boolean {
  if (!/blog_posts_grid/.test(path) || path.includes('.blocks.')) return false;
  const key = path.split('.').pop() ?? '';
  if (SECTION_LEVEL_FIELD_KEYS.has(key)) return true;
  return key in CONTENT_FIELD_TO_BLOCK;
}

export function blogPostsGridSidebarSelectionId(nodeId: string): string {
  if (!nodeId.startsWith('field:')) return nodeId;
  const path = nodeId.slice('field:'.length);
  if (!isBlogPostsGridContentFieldPath(path)) return nodeId;

  const settingsBase = path.replace(/\.[^.]+$/, '');
  const fieldKey = path.split('.').pop() ?? '';
  if (SECTION_LEVEL_FIELD_KEYS.has(fieldKey)) {
    const sectionMapped = blogPostsGridSectionSidebarNodeId(settingsBase);
    if (sectionMapped) return sectionMapped;
  }
  const blockSuffix = CONTENT_FIELD_TO_BLOCK[fieldKey];
  if (!blockSuffix) return nodeId;
  const mapped = blogPostsGridFieldSidebarNodeId(settingsBase, blockSuffix);
  return mapped ?? nodeId;
}

export function syntheticBlogPostsGridSidebarNode(
  nodeId: string,
  ...
  if (isBlogPostsGridCardGroupNodeId(nodeId)) {
    return { id: nodeId, label: 'Blog card', kind: 'block', icon: 'product-card' };
  }
  ...
  if (nodeId.startsWith('field:')) {
    const mapped = blogPostsGridSidebarSelectionId(nodeId);
    if (mapped !== nodeId) return syntheticBlogPostsGridSidebarNode(mapped, _editorSchema);
  }
