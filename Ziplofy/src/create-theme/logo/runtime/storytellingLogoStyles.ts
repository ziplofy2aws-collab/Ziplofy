...
export function storytellingLogoSizeVars(style: StorytellingLogoLayout): CSSProperties {
  const desktopHeight = style.sizeUnit === 'pixel' ? `${style.pixelHeight}px` : undefined;
  const desktopWidth = style.sizeUnit === 'percent' ? `${style.percentWidth}%` : undefined;
  ...
  return {
    ...(desktopHeight ? { ['--logo-height' as string]: desktopHeight } : {}),
    ...(desktopWidth ? { ['--logo-width' as string]: desktopWidth } : {}),
    ...(mobileHeight ? { ['--logo-height-mobile' as string]: mobileHeight } : {}),
    ...(mobileWidth ? { ['--logo-width-mobile' as string]: mobileWidth } : {}),
  };
}
  ...
  const mobile = `
    @media (max-width: 749px) {
      ${scope} [data-logo-mark] {
        max-height: var(--logo-height-mobile, var(--logo-height, 48px)) !important;
        width: var(--logo-width-mobile, var(--logo-width, auto)) !important;
        font-size: var(--logo-height-mobile, var(--logo-height, 48px)) !important;
      }
    }
  `;
  const custom = customCss.trim() ? `${scope} { ${customCss} }` : '';
