...
export function applyBlogPostsEditorialPreset(section: Record<string, unknown>): void {
  if (section.type !== 'blog-posts-editorial') return;

  const settings = (section.settings ?? {}) as Record<string, unknown>;
  settings.catalogVariant = settings.catalogVariant ?? 'blog-posts-editorial';
  settings.heading = settings.heading ?? 'Blog posts';
  settings.blogHandle = settings.blogHandle ?? '';
  settings.layoutType = settings.layoutType ?? 'editorial';
