...
import { fetchStoreMediaBlobForEditor, uploadStoreMediaFile, resolveStoreMediaUrl } from '@/lib/store-media';
import {
  ...
  rotatedBounds,
} from '@/lib/informatic-theme/theme-editor-image-editor.utils';
...
export function ThemeEditorImageEditorSheet({
  ...
  const fetchBlobForEditor = useCallback(
    async (url: string) => {
      if (!storeId) throw new Error('Select a store to edit store media images');
      return fetchStoreMediaBlobForEditor(storeId, url);
    },
    [storeId]
  );

  const imageFetchOptions = useMemo<ImageEditorFetchOptions | undefined>(
    () => (storeId ? { fetchBlob: fetchBlobForEditor } : undefined),
    [storeId, fetchBlobForEditor]
  );
  ...
  useEffect(() => {
    ...
    loadEditableImage(imageUrl.trim(), imageFetchOptions)
      .then((loaded) => {
        if (cancelled) {
  ...
  }, [open, imageUrl, applySnapshot, imageFetchOptions]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isBitmapExportable(source)) {
        ...
        const fallbackUrl =
          ...
          (source.src.startsWith('blob:') || source.src.startsWith('data:'))
            ? source.src
            : imageUrl);
        const exportable = await ensureExportableImage(source, fallbackUrl, imageFetchOptions);
      ...
      const uploaded = await uploadStoreMediaFile(storeId, file);
      const objectUrl = resolveStoreMediaUrl(uploaded);
