...
function buildSettingsFields(prefix) {
  return [
    field(prefix, 'sectionLayout', {
      ...
      sidebar: true,
      options: [{ value: 'with-hints', label: 'With hints' }],
    }),
    field(prefix, 'fullWidthOnMobile', {
      type: 'boolean',
...
const defaultSettings = {
  ...
  sectionLayout: 'with-hints',
  fullWidthOnMobile: false,
  gap: 18,
  cornerRadius: 20,
  mediaHeight: 'medium',
  contentPosition: 'below-media',
...
function patchFiles() {
  ...
  schema.layout.slideshow_inset = buildSection(
    'slideshow_inset',
    'Slideshow: Inset',
    'sections.slideshow_inset.settings'
  );

  const indexTpl = schema.templates.find((t) => t.id === 'index');
  if (indexTpl && !indexTpl.sections.some((s) => s.id === 'slideshow_inset')) {
    indexTpl.sections.push(
      buildSection(
        'slideshow_inset',
        'Slideshow: Inset',
        'templates.index.sections.slideshow_inset.settings'
      )
    );
  }
  ...
  if (!cfg.templates.index.sections.slideshow_inset) {
    cfg.templates.index.sections.slideshow_inset = {
      type: 'slideshow-inset',
      enabled: true,
      settings: { ...defaultSettings },
  ...
  console.log('added slideshow_inset pack');
}
