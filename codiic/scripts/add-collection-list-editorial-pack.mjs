...
const TILE_VARIANTS = ['folded-shirts', 'hanger-shirts', 'hanging-sweaters', 'clothing-rack'];
...
function patchFiles() {
  ...
  schema.layout.collection_list_editorial = buildSection(
    'collection_list_editorial',
    'Collection list: Editorial',
    'sections.collection_list_editorial.settings'
  );

  const indexTpl = schema.templates.find((t) => t.id === 'index');
  if (indexTpl && !indexTpl.sections.some((s) => s.id === 'collection_list_editorial')) {
    indexTpl.sections.push(
      buildSection(
        'collection_list_editorial',
        'Collection list: Editorial',
        'templates.index.sections.collection_list_editorial.settings'
      )
    );
  }
  ...
  cfg.sections.collection_list_editorial = {
    id: 'collection_list_editorial',
    type: 'collection-list-editorial',
    enabled: true,
  ...
  if (!cfg.templates.index.sections.collection_list_editorial) {
    cfg.templates.index.sections.collection_list_editorial = {
      type: 'collection-list-editorial',
      enabled: true,
      settings: { ...defaultSettings },
  ...
  const z3b = path.join(__dirname, '../../codiic-server/src/theme-packs/horizon');
  ...
  console.log('added collection_list_editorial pack');
}
