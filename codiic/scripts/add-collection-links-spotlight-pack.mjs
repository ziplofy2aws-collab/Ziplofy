...
function patchFiles() {
  ...
  schema.layout.collection_links_spotlight = buildSection(
    'collection_links_spotlight',
    'Collection links: Spotlight',
    'sections.collection_links_spotlight.settings'
  );

  const indexTpl = schema.templates.find((t) => t.id === 'index');
  if (indexTpl && !indexTpl.sections.some((s) => s.id === 'collection_links_spotlight')) {
    indexTpl.sections.push(
      buildSection(
        'collection_links_spotlight',
        'Collection links: Spotlight',
        'templates.index.sections.collection_links_spotlight.settings'
      )
    );
  }
  ...
  cfg.sections.collection_links_spotlight = {
    id: 'collection_links_spotlight',
    type: 'collection-links-spotlight',
    enabled: true,
  ...
  if (!cfg.templates.index.sections.collection_links_spotlight) {
    cfg.templates.index.sections.collection_links_spotlight = {
      type: 'collection-links-spotlight',
      enabled: true,
      settings: { ...defaultSettings },
  ...
  console.log('added collection_links_spotlight pack');
}
