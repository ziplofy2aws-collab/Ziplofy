...
const defaultSettings = {
  ...
  navigationIconBackground: 'circle',
  sectionWidth: 'page',
  layoutGap: 12,
...
function patchFiles() {
  ...
  schema.layout.collection_list_carousel = buildSection(
    'collection_list_carousel',
    'Collection list: Carousel',
    'sections.collection_list_carousel.settings'
  );

  const indexTpl = schema.templates.find((t) => t.id === 'index');
  if (indexTpl && !indexTpl.sections.some((s) => s.id === 'collection_list_carousel')) {
    indexTpl.sections.push(
      buildSection(
        'collection_list_carousel',
        'Collection list: Carousel',
        'templates.index.sections.collection_list_carousel.settings'
      )
    );
  }
  ...
  cfg.sections.collection_list_carousel = {
  ...
  if (!cfg.templates.index.sections.collection_list_carousel) {
    cfg.templates.index.sections.collection_list_carousel = {
      type: 'collection-list-carousel',
      enabled: true,
      settings: { ...defaultSettings },
  ...
  console.log('added collection_list_carousel pack');
}
