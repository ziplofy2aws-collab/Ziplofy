    ...
    blocks: [
      {
        ...
        settingsFields: [
          ...
          {
            path: `${settingsPrefix.replace('.settings', '.blocks')}.collection_tile.settings.columnSpan`,
            type: 'select',
            label: 'Width',
            widget: 'segmented',
            sidebar: true,
...
function patchFiles() {
  ...
  schema.layout.collection_list_bento = buildSection(
    'collection_list_bento',
    'Collection list: Bento',
    'sections.collection_list_bento.settings'
  );

  const indexTpl = schema.templates.find((t) => t.id === 'index');
  if (indexTpl && !indexTpl.sections.some((s) => s.id === 'collection_list_bento')) {
    indexTpl.sections.push(
      buildSection(
        'collection_list_bento',
        'Collection list: Bento',
        'templates.index.sections.collection_list_bento.settings'
      )
    );
  }
  ...
  cfg.sections.collection_list_bento = {
  ...
  if (!cfg.templates.index.sections.collection_list_bento) {
    cfg.templates.index.sections.collection_list_bento = {
      type: 'collection-list-bento',
      enabled: true,
      settings: { ...defaultSettings },
  ...
  console.log('added collection_list_bento pack');
}
