...
/** Shopify-style Editorial settings sheet order (flat general controls, then padding, then CSS). */
...
export function isEditorialSettingsPanelFields(fields: EditorFieldDef[]): boolean {
  ...
  return (
    keys.has('mediaPosition') &&
    keys.has('mediaWidth') &&
    !keys.has('textPosition') &&
    (keys.has('subheading') || keys.has('linkLabel'))
  );
}

export function prepareEditorialSettingsNode(node: SidebarNode): SidebarNode {
  const fields = sortEditorialPanelFields(
