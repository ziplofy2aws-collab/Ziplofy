...
type CollectionAddedBannerProps = {
  collectionTitle: string;
  onDismiss: () => void;
  onAddAnother: () => void;
};

const CollectionAddedBanner: React.FC<CollectionAddedBannerProps> = ({
  collectionTitle,
  onDismiss,
  onAddAnother,
}) => {
    ...
    >
      <div className="flex items-center justify-between gap-3 bg-emerald-700 px-4 py-2.5 text-white">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <span className="truncate text-[13px] font-semibold">Added {collectionTitle}</span>
        </div>
        <button
          type="button"
          onClick={onDismiss}
...
export default CollectionAddedBanner;
