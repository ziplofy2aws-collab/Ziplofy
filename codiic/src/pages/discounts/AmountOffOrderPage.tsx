...
import { amountOffOrderDetailsPath } from "../../utils/discount-navigation.util";
    ...
    try {
      ...
      if (editId) {
        ...
        if (res.success && res.data?._id) {
          navigate(amountOffOrderDetailsPath(res.data._id));
        }
      }
    } catch (err) {
    ...
    <div className="w-full">
      <div className="mx-auto max-w-[1200px] px-3 py-4 sm:px-4">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <DiscountFormHeader
            title={editId ? 'Edit amount off order' : 'Amount off order'}
            onBack={handleCancel}
            ...
            <div className="px-5 py-4 sm:px-6 sm:py-5">
              ...
              {formData.valueType === 'percentage' && (
                <div className="mt-4">
                  ...
                  <p className="mt-1.5 text-xs text-admin-text-secondary">Percentage off the order total</p>
                </div>
              )}
              {formData.valueType === 'fixed-amount' && (
                <div className="mt-4">
                  <label className="block text-xs font-medium text-admin-text-secondary uppercase tracking-wide mb-1.5">Fixed amount (₹)</label>
                  <div className="relative">
                  ...
                  <p className="mt-1.5 text-xs text-admin-text-secondary">Fixed amount off the order total in rupees</p>
                </div>
              )}
            </div>
          </div>
