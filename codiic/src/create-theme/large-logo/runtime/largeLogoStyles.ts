...
export function scopedLargeLogoMobileCss(scopeClass: string): string {
  return mobileMedia(
    `.${scopeClass} { padding-left: 20px !important; padding-right: 20px !important; }` +
      `.${scopeClass} .codiic-large-logo-corner { max-width: 100% !important; font-size: 14px !important; }` +
      `.${scopeClass} .codiic-large-logo-stage h1 { font-size: clamp(2.75rem, 14vw, 6rem) !important; }` +
      `.${scopeClass} .codiic-large-logo-stage { min-height: 200px !important; padding-top: 24px !important; }` +
