...
export const IMAGE_COMPARE_TEXT_GROUP_PANEL_GROUP_ORDER = [
  'Layout',
  'Size',
  'Appearance',
  'Borders',
...
const PANEL_GROUPS = new Set<string>(IMAGE_COMPARE_TEXT_GROUP_PANEL_GROUP_ORDER);
...
export const IMAGE_COMPARE_TEXT_GROUP_FIELD_KEYS = new Set([
  'direction',
  'layoutAlignment',
  'position',
  'layoutGap',
...
export function imageCompareTextGroupFieldDefs(settingsBase: string): EditorFieldDef[] {
...
export function imageCompareTextGroupCustomSizeFieldDefs(settingsBase: string): EditorFieldDef[] {
  const s = (key: string) => `${textGroupBase(settingsBase)}.${key}`;
...
export function imageCompareTextGroupFieldDefsFromNodeId(nodeId: string): EditorFieldDef[] {
  const sectionBase = imageCompareSectionBaseFromNodeId(nodeId);
  if (!sectionBase) return [];
  return [
    ...imageCompareTextGroupFieldDefs(sectionBase),
    ...imageCompareTextGroupCustomSizeFieldDefs(sectionBase),
  ];
}

export function pickImageCompareTextGroupField(
...
function resolvePercentSliderField(
  ...
  const existing = pickImageCompareTextGroupField(fields, key);
  if (existing) {
    return {
      ...existing,
      label,
...
export function resolveImageCompareTextGroupCustomWidthField(
  fields: EditorFieldDef[],
  anchor: EditorFieldDef | undefined,
...
export function resolveImageCompareTextGroupCustomHeightField(
  fields: EditorFieldDef[],
  anchor: EditorFieldDef | undefined
): EditorFieldDef | null {
...
export function isImageCompareTextGroupPanelField(field: EditorFieldDef): boolean {
  const key = field.path.split('.').pop() ?? '';
  if (!IMAGE_COMPARE_TEXT_GROUP_FIELD_KEYS.has(key)) return false;
...
export function isImageCompareTextGroupPanelFields(fields: EditorFieldDef[]): boolean {
  if (!fields.length) return false;
  return fields.every(isImageCompareTextGroupPanelField);
}

export function groupImageCompareTextGroupPanelFields(
  fields: EditorFieldDef[]
): Map<string, EditorFieldDef[]> {
  const map = new Map<string, EditorFieldDef[]>();
  for (const field of fields.filter(isImageCompareTextGroupPanelField)) {
...
export function prepareImageCompareTextGroupSettingsNode(node: SidebarNode): SidebarNode {
  const built = imageCompareTextGroupFieldDefsFromNodeId(node.id);
...
export const IMAGE_COMPARE_TEXT_GROUP_DEFAULTS: Record<string, string | boolean> = Object.fromEntries(
  Object.entries(imageCompareTextGroupDefaultSettings()).map(([k, v]) => [
    k,
...
export function extendImageCompareTextGroupValues(
  ...
  for (const field of fields) {
    ...
    const key = field.path.split('.').pop() ?? '';
    const fallback = IMAGE_COMPARE_TEXT_GROUP_DEFAULTS[key];
    if (fallback !== undefined) next[field.path] = fallback;
  }
  return next;
}

export function isImageCompareTextGroupBlockNodeId(nodeId: string): boolean {
  return isImageCompareTextGroupNodeId(nodeId);
}
