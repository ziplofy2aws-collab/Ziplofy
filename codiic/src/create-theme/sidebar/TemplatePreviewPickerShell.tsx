...
export type TemplatePreviewPickerShellProps = {
  ...
  /** Short label above the trigger, e.g. "Preview product". */
  label: string;
  open: boolean;
  onToggle: () => void;
  /** Main trigger content (thumbnail + title + path). */
  trigger: ReactNode;
  /** Optional trailing action inside the trigger (e.g. open storefront). */
  triggerAside?: ReactNode;
  /** Dropdown panel content when open. */
  children: ReactNode;
};

/** Shared chrome for product / collection / blog preview pickers in the theme editor sidebar. */
export function TemplatePreviewPickerShell({
  ...
  return (
    <div
      ref={rootRef}
      className="relative border-b border-[#e8e9eb] bg-linear-to-b from-white to-[#f8f9fa] px-3 py-3.5"
    >
...
export function TemplatePreviewPickerOption({
  ...
  return (
    ...
    >
      ...
      {selected ? (
        <CheckIcon className="h-4 w-4 shrink-0 text-[#005bd3]" aria-hidden />
      ) : null}
    </button>
  );
}
...
export const templatePreviewCreateClassName =
  'flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-[13px] font-semibold text-[#005bd3] transition hover:bg-[#eef3ff]';
