...
export type ThemeEditorEditMenuSheetProps = {
  open: boolean;
  menuId: string | null;
  onClose: () => void;
...
export function ThemeEditorEditMenuSheet({
  open,
  menuId,
  onClose,
  onSaved,
}: ThemeEditorEditMenuSheetProps) {
  const { activeStoreId } = useStore();
  const { fetchMenuById, updateMenu } = useStoreMenus();
  ...
  useEffect(() => {
    if (!open || !menuId || !activeStoreId) return;
  ...
  }, [open, menuId, activeStoreId, fetchMenuById]);
  ...
  return createPortal(
    <div className="fixed inset-0 z-[15000] flex flex-col justify-end" role="presentation">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="theme-editor-edit-menu-title"
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 pb-6 pt-2 sm:px-6">
          <h2
            id="theme-editor-edit-menu-title"
            className="pr-10 text-lg font-semibold tracking-tight text-gray-900"
          >
            Edit menu
          </h2>
          <p className="mt-1 text-sm text-gray-500">
            Update this menu’s name and links. Changes apply to your header navigation.
          </p>

          {loading ? (
            <div className="mt-8 space-y-4 animate-pulse" aria-busy="true" aria-label="Loading menu">
