export const ORDER_CONFIRMATION_EMAIL_SUBJECT = 'Order #{{order_number}} confirmed';
...
function lineItemRow(productName: string, quantity: number, price: string): string {
  return `
    <tr>
...
function fulfillmentGroup(title: string, subtitle: string, items: string): string {
  return `
    <tr>
...
export function buildSampleOrderItemsHtml(): string {
  return [
    lineItemRow('E-book: Fitness Guide', 1, 'Rs. 12.99'),
    fulfillmentGroup(
      'Shipping items',
      'Estimated delivery: <strong>Standard &middot; Friday, Jun 26&ndash;Sunday, Jun 28</strong>',
      [
    ...
    fulfillmentGroup(
      'Shipping items',
      'Estimated delivery: <strong>Standard &middot; Saturday, Jun 27&ndash;Monday, Jun 29</strong>',
      lineItemRow('Resistance bands', 1, 'Rs. 19.99')
    ),
    fulfillmentGroup(
      'Pickup in store items',
      'Estimated delivery &middot; <strong>Wednesday, Jun 24&ndash;Thursday, Jun 25</strong><br/>You&rsquo;ll receive an email when your order is ready for pickup.',
      lineItemRow('Protein powder', 1, 'Rs. 34.99')
    ),
    fulfillmentGroup('Shipping not required items', '', lineItemRow('Meditation app subscription', 1, 'Rs. 4.99')),
  ].join('');
}

export function buildOrderTotalsHtml(options?: {
  subtotal?: string;
  discount?: string;
  shipping?: string;
  pickup?: string;
...
export function buildSimpleOrderTotalsHtml(options: {
  subtotal: string;
  shipping: string;
  taxes: string;
  total: string;
...
export function buildCustomerInfoHtml(options?: {
...
export function buildOrderConfirmationEmailHtml(variables: Record<string, string>): string {
...
export function getOrderConfirmationEmailBodyTemplate(): string {
...
export const ORDER_CONFIRMATION_EMAIL_BODY = getOrderConfirmationEmailBodyTemplate();

export function formatOrderCurrency(amount: number, currency = 'Rs.'): string {
