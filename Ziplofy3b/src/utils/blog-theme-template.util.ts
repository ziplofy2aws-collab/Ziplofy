...
const TEMPLATE_PREFIX = "blogs";

export function isValidBlogThemeTemplate(value: unknown): value is string {
...
export function normalizeBlogThemeTemplate(value: unknown): string {
...
export function formatBlogThemeTemplateLabel(templateId: string): string {
...
export function resolveBlogJsonTemplateId(themeTemplate?: string | null): string {
  const normalized = normalizeBlogThemeTemplate(themeTemplate ?? DEFAULT_OPTION.value);
  if (normalized === DEFAULT_OPTION.value) return TEMPLATE_PREFIX;
  return normalized;
}

function addTemplateOption(
  options: BlogThemeTemplateOption[],
  seen: Set<string>,
  templateId: string,
  label?: string
): void {
  const normalized = normalizeBlogThemeTemplate(
  ...
  options.push({
    value: normalized,
    label: label?.trim() || formatBlogThemeTemplateLabel(normalized),
  });
}
...
export async function listBlogThemeTemplatesForStore(
  storeId: string
): Promise<BlogThemeTemplateOption[]> {
  const options: BlogThemeTemplateOption[] = [DEFAULT_OPTION];
