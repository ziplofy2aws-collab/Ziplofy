...
async function enforceAddToCartLimit(storeId: string | Types.ObjectId, quantity: number) {
  const settings = await CheckoutSettings.findOne({ storeId })
    .select('addToCartLimit')
    .lean();
  if (!settings?.addToCartLimit?.enabled) return;

  const limit = settings.addToCartLimit.useRecommended
    ? RECOMMENDED_ADD_TO_CART_LIMIT
    : settings.addToCartLimit.limit ?? RECOMMENDED_ADD_TO_CART_LIMIT;
  if (typeof limit === 'number' && quantity > limit) {
    throw new CustomError(`You can add a maximum of ${limit} of this item to the cart`, 400);
  }
}
...
export const createCartEntry = asyncErrorHandler(async (req: Request, res: Response) => {
  ...
  await enforceAddToCartLimit(storeId, qty);
...
export const updateCartEntry = asyncErrorHandler(async (req: Request, res: Response) => {
  ...
  const existing = await Cart.findOne({
    ...
    .select('storeId')
    .lean();
  if (!existing) throw new CustomError('Cart entry not found', 404);

  await enforceAddToCartLimit(existing.storeId, qty);
