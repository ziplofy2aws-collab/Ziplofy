...
async function fetchCountriesNowStates(): Promise<Map<string, CountryState[]>> {
  ...
  try {
    ...
    body.data.forEach((country: any) => {
      if (!country || !country.iso2 || !Array.isArray(country.states)) return;
      const states: CountryState[] = country.states
...
const knownStatesData: Record<string, Array<{ name: string; code: string; type?: string }>> = {
  US: [
...
function normalizeStateCode(name: string, code?: string): string {
  let normalized = (code || '')
    .toUpperCase()
    .replace(/[^A-Z0-9]/g, '');
...
function sanitizeStates(
  ...
  return states
    ...
    .filter(Boolean) as Array<{ name: string; code: string; type: string }>;
}
...
async function fetchStatesFromGeonames(countryIso2: string, username: string = 'demo'): Promise<Array<{ name: string; code: string }> | null> {
  try {
    ...
    if (statesData.geonames && Array.isArray(statesData.geonames)) {
      return statesData.geonames
        .filter((item: any) => item.fcode === 'ADM1') // Administrative division level 1
        .map((item: any) => ({
          name: item.name,
...
async function fetchStatesFromRestCountries(countryIso2: string): Promise<Array<{ name: string; code: string }> | null> {
  try {
...
async function seedStates() {
  try {
    ...
    for (const country of countries) {
      ...
      try {
        const result = await State.bulkWrite(stateOps as any, { ordered: false });
